...
import cn.edu.jsu.ll.dao.Pagecontroller2;
...
public class Gradeoperate extends JFrame {
	...
	public static void main(String[] args) {
		Gradeoperate frame = new Gradeoperate();// 实例化窗体
	...
	public Gradeoperate() {
		...
		Collections.addAll(titles, "编号", "项目编号", "成绩");
		Vector<Vector> stuInfo = new Pagecontroller2().getPaegData();// 获取第一页的数据

//		使用静态数据创建DefaultTableModel数据模型
		model = new DefaultTableModel(stuInfo, titles) ;// 使用Vector装载表格数据模型
		...
		btnFind.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(key.length()!=0) {
					String sql="select * from grade where id="+key;
					model=new DefaultTableModel(new UserMatch().getSelectAll2(sql),titles);//设置数据模型
					table.setModel(model);											
				}else {
					model=new DefaultTableModel(new UserMatch().getSelectAll2(sql1),titles);//设置数据模型
					table.setModel(model);
				}
		...
		Delete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String sql="delete from Grade where id="+key;
				try {
		...
		btnNewButton.setBounds(246, 51, 95, 23);
